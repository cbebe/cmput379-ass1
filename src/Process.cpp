...
Process Process::from(std::string const &cmd, InputOptions options) {
  ...
  if (pid < 0) {
     ...
     * but that would mean i had to call delete on the array and its c strings
     * after calling execvp which is probably impossible since the program is
     * already replaced
     *
     * Ended up using this:
    ...
    // still have to use pointers, so i just ended up using const_cast to remove
    // the const
...
void Process::Sleep(int seconds) {
  if (seconds < 0) throw "Seconds must be a positive integer";

  sleep(seconds);
}

void Process::Kill() {
  if (kill(pid, SIGKILL) != 0) throw "Kill failed";
...
void Process::Resume() {
  if (this->status == RUNNING) throw "Process already running";

  if (kill(pid, SIGCONT) != 0) throw "Resume failed";

  this->status = RUNNING;
}

void Process::Suspend() {
  if (this->status == SUSPENDED) throw "Process already suspended";

  if (kill(pid, SIGSTOP) != 0) throw "Suspend failed";
