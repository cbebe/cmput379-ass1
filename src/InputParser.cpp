...
void InputParser::ReadInput() {
    ...
    if (!checkLineLength()) throw "Line too long";

    tokenize();

    if (!checkNumberArgs()) throw "Too many arguments";

    if (!checkArgsLength()) throw "Argument too long";
}
...
InputOptions InputParser::GetCmdOptions() const {
    ...
    // other time
    if (options.cmdArgs.size() < 1) throw "No command given";
...
int InputParser::RequireInt(const char* message) {
    try {
        if (args.size() > 2) throw "Wrong number of args";
