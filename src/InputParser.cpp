...
InputOptions
InputParser::GetCmdOptions () const
{
...
int
InputParser::RequireInt (const char *message)
{
  try
    {
      if (args.size () > 2)
...
std::string
InputParser::GetInput () const
{
  return input;
}

std::vector<std::string>
InputParser::GetArgs () const
{
  return args;
}
...
bool
InputParser::checkNumberArgs () const
{
  return args.size () <= MAX_ARGS + 1;
}

bool
InputParser::checkLineLength () const
{
  return input.size () <= LINE_LENGTH;
}

bool
InputParser::checkArgsLength () const
{
